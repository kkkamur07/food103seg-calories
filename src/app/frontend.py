...
BENTOML_SEGMENT_ENDPOINT = BENTOML_SERVICE_URL + "segment"
...
def check_bentoml_connection():
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(
                f"BentoML service connection failed: Status {response.status_code}"
            )
            return False
        ...
        st.error(
            "BentoML service connection timed out. The backend might be slow to respond."
        )
        return False
...
def call_bentoml_segmentation(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(
                f"Failed to get segmentation from BentoML. Status Code: {response.status_code}"
            )
            try:
    ...
    # If connection to BentoML service was not sucessful
        ...
        st.error(
            "BentoML service request timed out. The model might be taking too long to process the image."
        )
        return None
    except Exception as e:
...
if bentoml_is_connected:
    ...
    if uploaded_file is not None:
        with col_button:
            ...
            segment_button = st.button(
                "Segment Food Items", use_container_width=True
            )  # button is filled to the column
        ...
        if segment_button:
            ...
            with st.spinner(
                "Processing image and performing segmentation via BentoML..."
            ):
            ...
            if segmented_image:
                ...
                st.warning(
                    "Could not display segmented image due to an error during processing by BentoML."
                )
    ...
    st.warning(
        "Cannot proceed. Please ensure the BentoML service is running and accessible."
    )
